...
function shallowCopy(obj) {
  const copy = {};
  const returnedCopy = Object.assign(copy, obj);
  return returnedCopy;
}
...
function compareObjects(obj1, obj2) {
  return (
    JSON.stringify(Object.entries(obj1)) ===
    JSON.stringify(Object.entries(obj2))
  );
}
...
function makeWord(lettersObject) {
  ...
  Object.entries(lettersObject).forEach(([char, indices]) => {
...
function sellTickets(queue) {
  ...
  const booleanArray = queue.map((item) => {
    switch (item) {
      case 25:
        change[25] += 1;
        return true;
...
function Rectangle(width, height) {
  ...
  const r = new RectangleClass(width, height);
  return r;
}
...
function fromJSON(proto, json) {
  ...
  const r = Object.create(proto.constructor.prototype);
  Object.assign(r, obj);
  return r;
}
...
function sortCitiesArray(arr) {
  return arr.sort((a, b) => {
    ...
    // If countries are equal, compare by city
    if (a.city < b.city) return -1;
    if (a.city > b.city) return 1;

    // If both country and city are equal, return 0 (no change in order)
    return 0;
  });
}
